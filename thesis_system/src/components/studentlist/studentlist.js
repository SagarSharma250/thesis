...
const Studentlist = () => {
  ...
  const getstudentinfo = () => {
    ...
    console.log(date);
    axios
      .post("http://localhost:80/thesis/student/upload.php/", {
        yeardate: date,
        yearmonth: month,
      })
      .then((response) => {
  ...
  const deleteinfo = (id) => {
    axios
      .post("http://localhost:80/thesis/student/delete.php/", { id: id })
      .then((response) => {
        console.log(response);
      ...
      {studentinfo.map((item) => (
        <ul className="listcontainer">
          <div className="infobox">
            <li className="text">Roll No: {item.roll}</li>
            <li className="text">
              Name: {item.firstname} {item.lastname}
            </li>

            <li className="text">Title: {item.thesis}</li>
          </div>
          ...
          <li>
            <button className="button_delete" onClick={() => ondelete(item.id)}>
              Delete
            </button>
          </li>
        </ul>
